...
import { mount_component } from "svelte/internal";
    ...
    const temp_to_combo = (temp_shifts) => {
        ...
        for (let i = 0; i < temp_shifts.length; i++) {
            if (
                temp_shifts[i].type === "(Duty Room)" &&
                temp_shifts[i + 1].type === "(Leader)" &&
                temp_shifts[i + 1].time === temp_shifts[i].time &&
                temp_shifts[i + 1].date === temp_shifts[i].date
            ) {
                let new_combo = {
                    ...temp_shifts[i],
                    leader_shift_id: temp_shifts[i + 1].shift_id,
                    leader_vols: temp_shifts[i + 1].vols,
                    leader_vol_shift_id: temp_shifts[i + 1].vol_shift_id,
                }
    ...
    const viewShifts = (start_date = null) => {
        ...
        let combo_shifts = $RotaStore["Shifts"]
...
<InteractMenu bind:showMenu {pos} on:rota_updated={handleUpdate} />
                    ...
                    <div class="font-semibold">
                        <div class="flex justify-center text-green-700 text-lg">
                            <span on:click={() => handleAddShift(display_start[j])} class="h-4 w-4 cursor-pointer"
                                ><svelte:component this={plus} /></span
                            >
                        </div>
